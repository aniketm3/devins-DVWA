...
function dvwaDatabaseConnect() {
	...
	// Connection timeout from config, default 10 seconds
	...
	if( $DBMS == 'MySQL' ) {
		// Reuse existing MySQLi connection if still alive
		$mysqli_alive = isset($GLOBALS["___mysqli_ston"]) && $GLOBALS["___mysqli_ston"] instanceof mysqli && @$GLOBALS["___mysqli_ston"]->ping();

		if( !$mysqli_alive ) {
			// Clean up stale MySQLi connection before reconnecting
			if( isset($GLOBALS["___mysqli_ston"]) && $GLOBALS["___mysqli_ston"] instanceof mysqli ) {
				@mysqli_close($GLOBALS["___mysqli_ston"]);
				$GLOBALS["___mysqli_ston"] = null;
			}

			// Attempt MySQLi connection with retry logic
			$connected = false;
			for( $attempt = 1; $attempt <= $max_retries; $attempt++ ) {
				...
				if( $GLOBALS["___mysqli_ston"] ) {
					...
					&& @((bool)mysqli_query($GLOBALS["___mysqli_ston"], "USE " . $_DVWA['db_database'])) ) {
						// Set session wait_timeout so MySQL cleans up idle connections
		...
		// Reuse existing PDO connection if still alive, otherwise (re)create it
		$pdo_alive = false;
 ...
 * Registered as a shutdown function to ensure cleanup on request end.
 */
